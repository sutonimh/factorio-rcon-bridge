...
def stop():
    ...
    Also clears storage.fle_travel: the server-side travel walker (travel.py) re-sets
    walking_state every 5 ticks, so halting without clearing the queue wouldn't stick."""
    return _print("/sc storage.fle_travel=nil; storage.derpface.walking_state={walking=false}")
